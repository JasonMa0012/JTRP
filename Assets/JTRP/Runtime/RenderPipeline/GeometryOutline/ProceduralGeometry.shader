Shader "Hidden/GeometryOutline"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 ClipPos2ScreenPos(float4 clipPos)
            {
                float4 screenPos = float4(clipPos.xy * 0.5 + 0.5, clipPos.zw);
                #if UNITY_UV_STARTS_AT_TOP
                screenPos.y = 1 - screenPos.y;
                #endif
                screenPos.xy = screenPos.xy * _ScreenParams.xy;
                return screenPos;
            }

            float4 ClipPos2ScreenPos3X(float4 clipPos)
            {
                float4 screenPos = float4(clipPos.xy * 0.5 + 0.5, clipPos.zw);
                #if UNITY_UV_STARTS_AT_TOP
                screenPos.y = 1 - screenPos.y;
                #endif
                screenPos.xy = screenPos.xy * _ScreenParams.xy * 3;
                return screenPos;
            }

            float LoadCamera3XDepth(uint2 pixelCoords)
            {
                return LOAD_TEXTURE2D_X_LOD(_Camera3XDepthTexture, pixelCoords, 0).r;
            }
            ...
            */
            ...
            void geometry_shader(uint pid: SV_PrimitiveID, point v2g input[1], inout TriangleStream<g2f> stream)
            {
                ...
                float PctExtend = 0.00; // 线段两端延伸的长度
            ...
            float4 fragment_shader(g2f i): SV_Target
            {
                ...
                float4 screenPos = ClipPos2ScreenPos3X(i.spinePos);
                #if UNITY_UV_STARTS_AT_TOP
                ...
                {
                    // float depth = LoadCameraDepth(screenPos.xy + i.normal * 1.0 * (index - floor(SAMPLE_COUNT * 0.5)));
                    float depth = LoadCameraDepth(screenPos.xy + offset[index]);
                    diff -= step(i.position.z + 0.0, depth);
                }*/
                if (_enable3XDepth)
                {
                    float2 bilinearPosX = screenPos.xy, bilinearPosY = screenPos.xy, bilinearPos = floor(screenPos.xy) +
                               0.5;
                    bilinearPosX.x = frac(bilinearPosX.x) >= 0.5
                                         ? floor(bilinearPosX.x) + 1.5
                                         : floor(bilinearPosX.x) - 0.5;
                    bilinearPosY.y = frac(bilinearPosY.y) >= 0.5
                                         ? floor(bilinearPosY.y) + 1.5
                                         : floor(bilinearPosY.y) - 0.5;
                    float2 bilinearPosXY = float2(bilinearPosX.x, bilinearPosY.y);
                    float depth = LoadCamera3XDepth(bilinearPos);
                    float depthX = LoadCamera3XDepth(bilinearPosX);
                    float depthY = LoadCamera3XDepth(bilinearPosY);
                    float depthXY = LoadCamera3XDepth(bilinearPosXY);

                    float xFactor = (screenPos.x - bilinearPos.x) / (bilinearPosX.x - bilinearPos.x);
                    float yFactor = (screenPos.y - bilinearPos.y) / (bilinearPosY.y - bilinearPos.y);
                    float x0 = lerp(depth, depthX, xFactor);
                    float x1 = lerp(depthY, depthXY, xFactor);
                    float visibility = step(lerp(x0, x1, yFactor), screenPos.z);
                ...
                // float depth = LoadCameraDepth(screenPos.xy);
                // clip(diff - 1);
                // outColor.a *= (float)diff / (float)SAMPLE_COUNT;
                // outColor.rgba = float4(screenPos.xy + i.normal, depth, i.position.z);
                return outColor;
