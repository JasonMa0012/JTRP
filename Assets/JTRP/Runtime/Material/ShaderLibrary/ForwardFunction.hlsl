...
float3 ChooseColor(float3 baseColorHSV, float4 OColor, float4 TColor)
{
    #ifdef _HSV_OTColor // 由cpu计算好hsv传入，由于unity会自动对颜色进行gamma校正所以反向校正
        // float3 TCHSV = LinearToSRGB(TColor.rgb);
        // float3 OCHSV = LinearToSRGB(OColor.rgb);
        float3 TCHSV = TColor.rgb;
        float3 OCHSV = OColor.rgb;
    #else
        float3 TCHSV = RgbToHsv(TColor.rgb);
        float3 OCHSV = RgbToHsv(OColor.rgb);
    #endif
    float3 TC = float3(TCHSV.r, baseColorHSV.gb * (TCHSV.gb / max(0.01, OCHSV.gb)));
    
    #ifdef _HSV_OTColor
        float3 c = HsvToRgb(TC);
        return c;
    #else
        return(float3)0;
        return HsvToRgb(TC);
...
float3 ChooseColor(float2 UV, float3 baseColor, float blend)
{
    if (blend == 0.0)
        return baseColor;
    UV = frac(UV);
    ...
    {
        if(UV.y < 0.5)
            c = lerp(c, ChooseColor(baseColor, _OColor1, _Color1), blend);
        else
        c = lerp(c, ChooseColor(baseColor, _OColor3, _Color3), blend);
    }
    else
    {
        if(UV.y < 0.5)
            c = lerp(c, ChooseColor(baseColor, _OColor2, _Color2), blend);
        else
        c = lerp(c, ChooseColor(baseColor, _OColor4, _Color4), blend);
    }
